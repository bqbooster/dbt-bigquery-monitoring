...
Monitoring incremental models use `on_schema_change: ignore` because dbt-bigquery can hang when `copy_partitions` is combined with automatic schema evolution. After package upgrades that add columns, run `dbt run --full-refresh` on affected models. To use `append_new_columns` instead, set `use_copy_partitions: false` in your project.
