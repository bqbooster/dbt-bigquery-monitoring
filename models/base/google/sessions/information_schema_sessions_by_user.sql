...
Access control with IAM. -#}
    ...
    FROM `{{ project | trim }}`.`region-{{ var('bq_region') }}`.`INFORMATION_SCHEMA`.`SESSIONS_BY_USER`
    ...
    SELECT
    creation_time, expiration_time, is_active, last_modified_time, project_id, project_number, session_id, user_email,
    FROM
    base
