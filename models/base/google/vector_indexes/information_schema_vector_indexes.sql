...
Access control with IAM. -#}
      ...
      {% if project_list()|length > 0 -%}
          {% for project in project_list() -%}
            SELECT index_catalog, index_schema, table_name, index_name, index_status, creation_time, last_modification_time, last_refresh_time, disable_time, disable_reason, ddl, coverage_percentage, unindexed_row_count, total_logical_bytes, total_storage_bytes
            FROM `{{ project | trim }}`.`region-{{ var('bq_region') }}`.`INFORMATION_SCHEMA`.`VECTOR_INDEXES`
