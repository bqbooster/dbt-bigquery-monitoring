...
SELECT
  ...
  s.project_number,
  MAX(s.storage_last_modified_time) storage_last_modified_time,
  SUM(s.total_rows) total_rows,
  SUM(s.total_partitions) total_partitions,
  SUM(s.total_logical_bytes) total_logical_bytes,
  SUM(IF(deleted=false, s.active_logical_bytes, 0)) active_logical_bytes,
  SUM(IF(deleted=false, s.long_term_logical_bytes, 0)) long_term_logical_bytes,
  SUM(s.total_physical_bytes) total_physical_bytes,
  SUM(s.active_physical_bytes) active_physical_bytes,
  SUM(s.long_term_physical_bytes) long_term_physical_bytes,
  SUM(s.time_travel_physical_bytes) time_travel_physical_bytes,
  SUM(s.fail_safe_physical_bytes) fail_safe_physical_bytes,
  FROM {{ ref('information_schema_table_storage') }} s
  GROUP BY ALL
),
